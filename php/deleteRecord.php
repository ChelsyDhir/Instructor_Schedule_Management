...
the code in catch-block is executed. */
    ...
    try {
        $sql = "DELETE FROM $dbname.Instructor WHERE Instructor_id = :Iid";
        $stmnt = $conn->prepare($sql);     // read about prepared statements here: https://www.w3schools.com/php/php_mysql_prepared_statements.asp
        $stmnt->bindParam(':Iid', $_POST['insId']);   // :stdId is the variable that we used in $sql, there must be a colon (:) in front of it.
